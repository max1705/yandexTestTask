class ItcSlider {
  ...
  constructor(el, config = {}, prefix = 'itc-slider__') {
    ...
    this.#config = {
      loop: true, autoplay: false, interval: 4000, refresh: true, swipe: true, ...config
    };
  ...
  #balanceItems() {
    ...
    if (this.#state.direction === 'next') {
      ...
      if (exItemRectRight < wrapperRect.left - targetWidth) {
        ...
        const translate = this.#state.exTranslateMin + countItems * this.#state.width + 564;
        elFound.translate = translate;
				if (elFound.order-3 < 0) {
          document.getElementById("slideNumber").innerHTML = 6+(elFound.order-3)%6;
        }
				else
				{
          document.getElementById("slideNumber").innerHTML = (elFound.order-3)%6;
					if ((elFound.order-3)%6 == 0) 
						document.getElementById("slideNumber").innerHTML = 6;
				}
      ...
      if (exItemRectLeft > wrapperRect.right + targetWidth) {
        ...
        const translate = this.#state.exTranslateMax - countItems * this.#state.width - 564;
        elFound.translate = translate;
        if (elFound.order-3 < 0) {
          document.getElementById("slideNumber").innerHTML = 6+(elFound.order-3)%6;
        }
  ...
  #move() {
    const widthItem = this.#state.direction === 'next' ? -this.#state.width-94 : this.#state.width+94;
  ...
  #init() {
    ...
    document.getElementById("slideNumber").innerHTML = 1;
    ...
    if (this.#config.loop) {
      ...
      const translate = -(lastIndex + 1) * this.#state.width - 564;
